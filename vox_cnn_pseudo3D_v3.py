...
def voxCNN_pseudo3D_V3(
   ...
   # Block 1
    x = layers.Conv2D(
        16, (3, 3), activation='relu', padding='same', name='block1_conv1')(
            input_psudoRGB)
    x = layers.Conv2D(
        16, (3, 3), activation='relu', padding='same', name='block1_conv2')(x)
    # x = layers.Conv2D(
    #     16, (3, 3), activation='relu', padding='same', name='block1_conv3')(x)
    x = layers.MaxPooling2D((2, 2), name='block1_pool')(x)

    # Block 2
    x = layers.Conv2D(
        32, (3, 3), activation='relu', padding='same', name='block2_conv1')(x)
    x = layers.Conv2D(
        32, (3, 3), activation='relu', padding='same', name='block2_conv2')(x)
    # x = layers.Conv2D(
    #     32, (3, 3), activation='relu', padding='same', name='block2_conv3')(x)
    x = layers.MaxPooling2D((2, 2), name='block2_pool')(x)

    # Block 3
    x = layers.Conv2D(
        64, (3, 3), activation='relu', padding='same', name='block3_conv1')(x)
    x = layers.Conv2D(
        64, (3, 3), activation='relu', padding='same', name='block3_conv2')(x)
    # x = layers.Conv2D(
    #     64, (3, 3), activation='relu', padding='same', name='block3_conv3')(x)
    x = layers.MaxPooling2D((2, 2), name='block3_pool')(x)

    # # Block 4
    x = layers.Conv2D(
        128, (3, 3), activation='relu', padding='same', name='block4_conv1')(x)
    x = layers.Conv2D(
        128, (3, 3), activation='relu', padding='same', name='block4_conv2')(x)
    # # x = layers.Conv2D(
    # #     128, (3, 3), activation='relu', padding='same', name='block4_conv3')(x)
    ...
    # Create model.
    model = Model(img_input, x, name='VoxCNN_pseudoRGB_V3')
