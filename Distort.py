...
class Distort():
    """
    This class performs randomised, elastic distortions on image in 3D.
    """
    ...
    def perform_operation(self, image3D):
        """
        Distorts the passed image(s) according to the parameters supplied during
        instantiation, returning the newly distorted image.
        :param image3D: The image(s) to be distorted.
        :type image3D: List containing PIL.Image object(s).
        ...
        def do(slice):
            ...
            image.transform(image.size, Image.MESH, generated_mesh, resample=Image.BICUBIC)
            return np.array(image)

        augmented_image3D = []

        for slice in image3D:
            augmented_image3D.append(do(slice))

        return np.array(augmented_image3D)


class GaussianDistortion():
    """
    This class performs randomised, elastic gaussian distortions on image3D.
    """
    def __init__(self, grid_width, grid_height, magnitude, corner, method, mex, mey, sdx, sdy):
        """
        As well as the probability, the granularity of the distortions
        produced by this class can be controlled using the width and
        height of the overlaying distortion grid. The larger the height
    ...
    def perform_operation(self, image3D):
        ...
        def do(image3D):
            ...
            for slice in image3D:
                ...
                augmented_image3D.append(np.array(image))

            return augmented_image3D

        augmented_image3D = do(image3D)
